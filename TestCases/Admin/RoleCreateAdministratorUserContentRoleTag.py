...
class RoleCreateAdministratorUserContentRoleTag():
    
    def createCreateAdministratorUserContentRoleTag(self,RoleName,Description):
    ...
    def createCreateAdministratorUserContentRoleTagMain(self):
        from BaseTestClass import excelPath  
         try:   
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            sheet=book.sheet_by_name('Role')
            cell = sheet.cell(251,1)
            RoleName = cell.value
            cell = sheet.cell(252,1)
            Description = cell.value
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            first_sheet = book.sheet_by_name('UserAssignToRole')
            cell = first_sheet.cell(4,1)
            FirstName = cell.value
            obj2= RoleCreateAdministratorUserContentRoleTag()
            obj2.createCreateAdministratorUserContentRoleTag(RoleName,Description) 
...
if __name__ == '__main__':
    ...
    obj11= CreateAdministratorUserContentRoleTag()
    obj12= BaseTestClass()
    obj12.UserLogin()
    obj11.createCreateAdministratorUserContentRoleTagMain()
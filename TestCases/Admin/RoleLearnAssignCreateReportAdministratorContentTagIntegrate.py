...
class RoleLearnAssignCreateReportAdministratorContentTagIntegrate():
    
    def createLearnAssignCreateReportAdministratorContentTagIntegrate(self,RoleName,Description):
    ...
    def createLearnAssignCreateReportAdministratorContentTagIntegrateMain(self):
        from BaseTestClass import excelPath  
        try:   
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            sheet=book.sheet_by_name('Role')
            cell = sheet.cell(472,1)
            RoleName = cell.value
            cell = sheet.cell(473,1)
            Description = cell.value
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            first_sheet = book.sheet_by_name('UserAssignToRole')
            cell = first_sheet.cell(3,1)
            FirstName = cell.value
            obj2= RoleLearnAssignCreateReportAdministratorContentTagIntegrate()
            obj2.createLearnAssignCreateReportAdministratorContentTagIntegrate(RoleName,Description)  
...
if __name__ == '__main__':
    ...
    obj11= LearnAssignCreateReportAdministratorContentTagIntegrate()
    obj12= BaseTestClass()
    obj12.UserLogin()
    obj11.createLearnAssignCreateReportAdministratorContentTagIntegrateMain()
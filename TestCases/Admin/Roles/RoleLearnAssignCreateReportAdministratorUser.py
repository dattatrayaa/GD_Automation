...
class RoleLearnAssignCreateReportAdministratorUser():
    def createLearnAssignCreateReportAdministratorUser(self,RoleName,Description):
    ...
    def createLearnAssignCreateReportAdministratorUserMain(self):
        from BaseTestClass import excelPath  
        try:   
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            sheet=book.sheet_by_name('Role')
            cell = sheet.cell(424,5)
            RoleName = cell.value
            cell = sheet.cell(425,5)
            Description = cell.value
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            first_sheet = book.sheet_by_name('UserAssignToRole')
            cell = first_sheet.cell(3,1)
            FirstName = cell.value
            obj2= RoleLearnAssignCreateReportAdministratorUser()
            obj2.createLearnAssignCreateReportAdministratorUser(RoleName,Description)  
...
if __name__ == '__main__':
    ...
    obj11= LearnAssignCreateReportAdministratorUser()
    obj12= BaseTestClass()
    obj12.UserLogin()
    obj11.createLearnAssignCreateReportAdministratorUserMain()
...
class RoleLearnAdministratorUserRoleBrand():
    
    def createLearnAdministratorUserRoleBrand(self,RoleName,Description):
    ...
    def createLearnAdministratorUserRoleBrandMain(self):  
        try:   
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            sheet=book.sheet_by_name('Role')
            cell = sheet.cell(227,1)
            RoleName = cell.value
            cell = sheet.cell(228,1)
            Description = cell.value
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            first_sheet = book.sheet_by_name('UserAssignToRole')
            cell = first_sheet.cell(3,1)
            FirstName = cell.value
            obj2= RoleLearnAdministratorUserRoleBrand()
            obj2.createLearnAdministratorUserRoleBrand(RoleName,Description)  
...
if __name__ == '__main__':
    ...
    obj11= LearnAdministratorUserRoleBrand()
    obj12= BaseTestClass()
    obj12.UserLogin()
    obj11.createLearnAdministratorUserRoleBrandMain()
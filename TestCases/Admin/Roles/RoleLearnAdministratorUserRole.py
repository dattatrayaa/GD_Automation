...
class RoleLearnAdministratorUserRole():
    
    def createLearnAdministratorUserRole(self,RoleName,Description):
    ...
    def createLearnAdministratorUserRoleMain(self):
        from BaseTestClass import excelPath  
        try:   
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            sheet=book.sheet_by_name('Role')
            cell = sheet.cell(243,1)
            RoleName = cell.value
            cell = sheet.cell(244,1)
            Description = cell.value
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            first_sheet = book.sheet_by_name('UserAssignToRole')
            cell = first_sheet.cell(3,1)
            FirstName = cell.value
            obj2= RoleLearnAdministratorUserRole()
            obj2.createLearnAdministratorUserRole(RoleName,Description)  
...
if __name__ == '__main__':
    ...
    obj11= LearnAdministratorUserRole()
    obj12= BaseTestClass()
    obj12.UserLogin()
    obj11.createLearnAdministratorUserRoleMain()
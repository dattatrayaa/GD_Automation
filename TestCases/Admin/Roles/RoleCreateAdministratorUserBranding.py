...
class RoleCreateAdministratorUserBranding():
    
    def createCreateAdministratorUserBranding(self,RoleName,Description):
    ...
    def createCreateAdministratorUserBrandingMain(self):  
        try:   
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            sheet=book.sheet_by_name('Role')
            cell = sheet.cell(300,5)
            RoleName = cell.value
            cell = sheet.cell(301,5)
            Description = cell.value
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            first_sheet = book.sheet_by_name('UserAssignToRole')
            cell = first_sheet.cell(4,1)
            FirstName = cell.value
            obj2= RoleCreateAdministratorUserBranding()
            obj2.createCreateAdministratorUserBranding(RoleName,Description)  
...
if __name__ == '__main__':
    ...
    obj11= CreateAdministratorUserBranding()
    obj12= BaseTestClass()
    obj12.UserLogin()
    obj11.createCreateAdministratorUserBrandingMain()
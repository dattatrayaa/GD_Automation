...
class RoleLearnAdministratorTag():
    
    def createLearnAdministratorTag(self,RoleName,Description):
    ...
    def createLearnAdministratorTagMain(self):  
        try:   
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            sheet=book.sheet_by_name('Role')
            cell = sheet.cell(236,5)
            RoleName = cell.value
            cell = sheet.cell(237,5)
            Description = cell.value
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            first_sheet = book.sheet_by_name('UserAssignToRole')
            cell = first_sheet.cell(3,1)
            FirstName = cell.value
            obj2= RoleLearnAdministratorTag()
            obj2.createLearnAdministratorTag(RoleName,Description)
...
if __name__ == '__main__':
    ...
    obj11= LearnAdministratorTag()
    obj12= BaseTestClass()
    obj12.UserLogin()
    obj11.createLearnAdministratorTagMain()
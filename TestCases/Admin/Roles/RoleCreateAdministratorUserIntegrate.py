...
class RoleCreateAdministratorUserIntegrate():
    
    def createCreateAdministratorUserIntegrate(self,RoleName,Description):
    ...
    def createCreateAdministratorUserIntegrateMain(self): 
        from BaseTestClass import excelPath 
        try:   
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            sheet=book.sheet_by_name('Role')
            cell = sheet.cell(275,1)
            RoleName = cell.value
            cell = sheet.cell(276,1)
            Description = cell.value
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            first_sheet = book.sheet_by_name('UserAssignToRole')
            cell = first_sheet.cell(4,1)
            FirstName = cell.value
            obj2= RoleCreateAdministratorUserIntegrate()
            obj2.createCreateAdministratorUserIntegrate(RoleName,Description) 
...
if __name__ == '__main__':
    ...
    obj11= CreateAdministratorUserIntegrate()
    obj12= BaseTestClass()
    obj12.UserLogin()
    obj11.createCreateAdministratorUserIntegrateMain()
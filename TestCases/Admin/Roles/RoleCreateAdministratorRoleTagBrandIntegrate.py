...
class RoleCreateAdministratorRoleTagBrandIntegrate():
    
    def createCreateAdministratorRoleTagBrandIntegrate(self,RoleName,Description):
    ...
    def createCreateAdministratorRoleTagBrandIntegrateMain(self):
        from BaseTestClass import excelPath  
        try:   
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            sheet=book.sheet_by_name('Role')
            cell = sheet.cell(260,5)
            RoleName = cell.value
            cell = sheet.cell(261,5)
            Description = cell.value
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            first_sheet = book.sheet_by_name('UserAssignToRole')
            cell = first_sheet.cell(4,1)
            FirstName = cell.value
            obj2= RoleCreateAdministratorRoleTagBrandIntegrate()
            obj2.createCreateAdministratorRoleTagBrandIntegrate(RoleName,Description) 
...
if __name__ == '__main__':
    ...
    obj11= CreateAdministratorRoleTagBrandIntegrate()
    obj12= BaseTestClass()
    obj12.UserLogin()
    obj11.createCreateAdministratorRoleTagBrandIntegrateMain()
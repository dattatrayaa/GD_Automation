...
class RoleAssignAdministratorContentRoleTagBrand():
    
    def createAssignAdministratorContentRoleTagBrand(self,RoleName,Description):
    ...
    def createAssignAdministratorContentRoleTagBrandMain(self):  
        try:   
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            sheet=book.sheet_by_name('Role')
            cell = sheet.cell(328,1)
            RoleName = cell.value
            cell = sheet.cell(329,1)
            Description = cell.value
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            first_sheet = book.sheet_by_name('UserAssignToRole')
            cell = first_sheet.cell(1,1)
            FirstName = cell.value
            obj2= RoleAssignAdministratorContentRoleTagBrand()
            obj2.createAssignAdministratorContentRoleTagBrand(RoleName,Description)
...
if __name__ == '__main__':
    ...
    obj11= AssignAdministratorContentRoleTagBrand()
    obj12= BaseTestClass()
    obj12.UserLogin()
    obj11.createAssignAdministratorContentRoleTagBrandMain()
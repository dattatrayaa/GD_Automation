...
class RoleLearnAdministratorRoleTag():
    
    def createLearnAdministratorRoleTag(self,RoleName,Description):
    ...
    def createLearnAdministratorRoleTagMain(self):  
        try:   
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            sheet=book.sheet_by_name('Role')
            cell = sheet.cell(204,5)
            RoleName = cell.value
            cell = sheet.cell(205,5)
            Description = cell.value
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            first_sheet = book.sheet_by_name('UserAssignToRole')
            cell = first_sheet.cell(3,1)
            FirstName = cell.value
            obj2= RoleLearnAdministratorRoleTag()
            obj2.createLearnAdministratorRoleTag(RoleName,Description)
...
if __name__ == '__main__':
    ...
    obj11= LearnAdministratorRoleTag()
    obj12= BaseTestClass()
    obj12.UserLogin()
    obj11.createLearnAdministratorRoleTagMain()
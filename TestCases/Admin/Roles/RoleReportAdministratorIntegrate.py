...
class RoleReportAdministratorIntegrate():
    
    def createReportAdministratorIntegrate(self,RoleName,Description):
    ...
    def createReportAdministratorIntegrateMain(self):  
        try:   
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            sheet=book.sheet_by_name('Role')
            cell = sheet.cell(380,5)
            RoleName = cell.value
            cell = sheet.cell(381,5)
            Description = cell.value
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            first_sheet = book.sheet_by_name('UserAssignToRole')
            cell = first_sheet.cell(2,1)
            FirstName = cell.value
            obj2= RoleReportAdministratorIntegrate()
            obj2.createReportAdministratorIntegrate(RoleName,Description)  
...
if __name__ == '__main__':
    ...
    obj11= ReportAdministratorIntegrate()
    obj12= BaseTestClass()
    obj12.UserLogin()
    obj11.createReportAdministratorIntegrateMain()
...
class RoleAssignAdministratorUser():
    
    def createAssignAdministratorUser(self,RoleName,Description):
    ...
    def createAssignAdministratorUserMain(self):  
        try:   
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            sheet=book.sheet_by_name('Role')
            cell = sheet.cell(312,5)
            RoleName = cell.value
            cell = sheet.cell(313,5)
            Description = cell.value
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            first_sheet = book.sheet_by_name('UserAssignToRole')
            cell = first_sheet.cell(1,1)
            FirstName = cell.value
            obj2= RoleAssignAdministratorUser()
            obj2.createAssignAdministratorUser(RoleName,Description)
...
if __name__ == '__main__':
    ...
    obj11= AssignAdministratorUser()
    obj12= BaseTestClass()
    obj12.UserLogin()
    obj11.createAssignAdministratorUserMain()
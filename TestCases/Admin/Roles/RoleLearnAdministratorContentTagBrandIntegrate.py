...
class RoleLearnAdministratorContentTagBrandIntegrate():
    
    def createLearnAdministratorContentTagBrandIntegrate(self,RoleName,Description):
    ...
    def createLearnAdministratorContentTagBrandIntegrateMain(self):  
        try:   
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            sheet=book.sheet_by_name('Role')
            cell = sheet.cell(247,1)
            RoleName = cell.value
            cell = sheet.cell(248,1)
            Description = cell.value
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            first_sheet = book.sheet_by_name('UserAssignToRole')
            cell = first_sheet.cell(3,1)
            FirstName = cell.value
            obj2= RoleLearnAdministratorContentTagBrandIntegrate()
            obj2.createLearnAdministratorContentTagBrandIntegrate(RoleName,Description)  
...
if __name__ == '__main__':
    ...
    obj11= LearnAdministratorContentTagBrandIntegrate()
    obj12= BaseTestClass()
    obj12.UserLogin()
    obj11.createLearnAdministratorContentTagBrandIntegrateMain()
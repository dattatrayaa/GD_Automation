...
class RoleAssignAdministratorUserBranding():
    
    def createAssignAdministratorUserBranding(self,RoleName,Description):
    ...
    def createAssignAdministratorUserBrandingMain(self):
        from BaseTestClass import excelPath  
        try:   
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            sheet=book.sheet_by_name('Role')
            cell = sheet.cell(360,1)
            RoleName = cell.value
            cell = sheet.cell(361,1)
            Description = cell.value
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            first_sheet = book.sheet_by_name('UserAssignToRole')
            cell = first_sheet.cell(1,1)
            FirstName = cell.value
            obj2= RoleAssignAdministratorUserBranding()
            obj2.createAssignAdministratorUserBranding(RoleName,Description)
...
if __name__ == '__main__':
    ...
    obj11= AssignAdministratorUserBranding()
    obj12= BaseTestClass()
    obj12.UserLogin()
    obj11.createAssignAdministratorUserBrandingMain()
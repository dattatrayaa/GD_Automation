...
class RoleLearnAdministratorContentTagIntegrate():
    
    def createLearnAdministratorContentTagIntegrate(self,RoleName,Description):
    ...
    def createLearnAdministratorContentTagIntegrateMain(self):  
        try:   
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            sheet=book.sheet_by_name('Role')
            cell = sheet.cell(228,5)
            RoleName = cell.value
            cell = sheet.cell(229,5)
            Description = cell.value
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            first_sheet = book.sheet_by_name('UserAssignToRole')
            cell = first_sheet.cell(3,1)
            FirstName = cell.value
            obj2= RoleLearnAdministratorContentTagIntegrate()
            obj2.createLearnAdministratorContentTagIntegrate(RoleName,Description)  
...
if __name__ == '__main__':
    ...
    obj11= LearnAdministratorContentTagIntegrate()
    obj12= BaseTestClass()
    obj12.UserLogin()
    obj11.createLearnAdministratorContentTagIntegrateMain()
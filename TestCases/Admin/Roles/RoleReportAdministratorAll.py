...
class RoleReportAdministratorAll():
    
    def createReportAdministratorAllRole(self,RoleName,Description):
    ...
    def createReportAdministratorAllMain(self):  
        from BaseTestClass import excelPath
        try:   
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            sheet=book.sheet_by_name('Role')
            cell = sheet.cell(214,1)
            RoleName = cell.value
            cell = sheet.cell(215,1)
            Description = cell.value
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            first_sheet = book.sheet_by_name('UserAssignToRole')
            cell = first_sheet.cell(2,1)
            FirstName = cell.value
            obj2= RoleReportAdministratorAll()
            obj2.createReportAdministratorAllRole(RoleName,Description) 
...
if __name__ == '__main__':
    ...
    obj11= ReportAdministratorAll()
    obj12= BaseTestClass()
    obj12.UserLogin()
    obj11.createReportAdministratorAllMain()
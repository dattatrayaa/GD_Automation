...
class RoleAssignAdministratorRoleTagBrandIntegrate():
    
    def createAssignAdministratorRoleTagBrandIntegrate(self,RoleName,Description):
    ...
    def createAssignAdministratorRoleTagBrandIntegrateMain(self): 
        from BaseTestClass import excelPath 
        try:   
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            sheet=book.sheet_by_name('Role')
            cell = sheet.cell(316,5)
            RoleName = cell.value
            cell = sheet.cell(317,5)
            Description = cell.value
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            first_sheet = book.sheet_by_name('UserAssignToRole')
            cell = first_sheet.cell(1,1)
            FirstName = cell.value
            obj2= RoleAssignAdministratorRoleTagBrandIntegrate()
            obj2.createAssignAdministratorRoleTagBrandIntegrate(RoleName,Description)
...
if __name__ == '__main__':
    ...
    obj11= AssignAdministratorRoleTagBrandIntegrate()
    obj12= BaseTestClass()
    obj12.UserLogin()
    obj11.createAssignAdministratorRoleTagBrandIntegrateMain()
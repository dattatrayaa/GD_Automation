...
class ReportAdministratorUserTagBrandIntegrate():
    
    def createReportAdministratorUserTagBrandIntegrate(self,RoleName,Description):
    ...
    def maincreateReportAdminiUserTagBrandIntegrate(self): 
        from BaseTestClass import excelPath
        try:   
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            sheet=book.sheet_by_name('Role')
            cell = sheet.cell(404,1)
            RoleName = cell.value
            cell = sheet.cell(405,1)
            Description = cell.value
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            first_sheet = book.sheet_by_name('UserAssignToRole')
            cell = first_sheet.cell(2,1)
            FirstName = cell.value
            obj2= ReportAdministratorUserTagBrandIntegrate()
            obj2.createReportAdministratorUserTagBrandIntegrate(RoleName,Description)  
...
if __name__ == '__main__':
    ...
    obj11= ReportAdministratorUserTagBrandIntegrate()
    obj12= BaseTestClass()
    obj12.UserLogin()
    obj11.maincreateReportAdminiUserTagBrandIntegrate()
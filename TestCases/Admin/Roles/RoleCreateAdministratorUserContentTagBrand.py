...
class RoleCreateAdministratorUserContentTagBrand():
    
    def createCreateAdministratorUserContentTagBrand(self,RoleName,Description):
    ...
    def createCreateAdministratorUserContentTagBrandMain(self):  
        try:   
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            sheet=book.sheet_by_name('Role')
            cell = sheet.cell(287,1)
            RoleName = cell.value
            cell = sheet.cell(288,1)
            Description = cell.value
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            first_sheet = book.sheet_by_name('UserAssignToRole')
            cell = first_sheet.cell(4,1)
            FirstName = cell.value
            obj2= RoleCreateAdministratorUserContentTagBrand()
            obj2.createCreateAdministratorUserContentTagBrand(RoleName,Description)  
...
if __name__ == '__main__':
    ...
    obj11= CreateAdministratorUserContentTagBrand()
    obj12= BaseTestClass()
    obj12.UserLogin()
    obj11.createCreateAdministratorUserContentTagBrandMain()
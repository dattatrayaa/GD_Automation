...
class RoleReportAdministratorContentRoleTagBrand():
    
    def createReportAdministratorContentRoleTagBrand(self,RoleName,Description):
    ...
    def createReportAdministratorContentRoleTagBrandMain(self):  
        from BaseTestClass import excelPath
        try:   
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            sheet=book.sheet_by_name('Role')
            cell = sheet.cell(384,1)
            RoleName = cell.value
            cell = sheet.cell(385,1)
            Description = cell.value
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            first_sheet = book.sheet_by_name('UserAssignToRole')
            cell = first_sheet.cell(2,1)
            FirstName = cell.value
            obj2= RoleReportAdministratorContentRoleTagBrand()
            obj2.createReportAdministratorContentRoleTagBrand(RoleName,Description) 
...
if __name__ == '__main__':
    ...
    obj11= ReportAdministratorContentRoleTagBrand()
    obj12= BaseTestClass()
    obj12.UserLogin()
    obj11.createReportAdministratorContentRoleTagBrandMain()
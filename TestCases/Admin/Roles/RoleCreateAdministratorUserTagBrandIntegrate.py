...
class RoleCreateAdministratorUserTagBrandIntegrate():
    
    def createCreateAdministratorUserTagBrandIntegrate(self,RoleName,Description):
    ...
    def createCreateAdministratorUserTagBrandIntegrateMain(self):  
        try:   
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            sheet=book.sheet_by_name('Role')
            cell = sheet.cell(291,1)
            RoleName = cell.value
            cell = sheet.cell(292,1)
            Description = cell.value
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            first_sheet = book.sheet_by_name('UserAssignToRole')
            cell = first_sheet.cell(4,1)
            FirstName = cell.value
            obj2= RoleCreateAdministratorUserTagBrandIntegrate()
            obj2.createCreateAdministratorUserTagBrandIntegrate(RoleName,Description)  
...
if __name__ == '__main__':
    ...
    obj11= CreateAdministratorUserTagBrandIntegrate()
    obj12= BaseTestClass()
    obj12.UserLogin()
    obj11.createCreateAdministratorUserTagBrandIntegrateMain()
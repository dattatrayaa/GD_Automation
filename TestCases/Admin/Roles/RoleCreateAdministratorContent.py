...
class RoleCreateAdministratorContent():
    
    def RolecreateCreateAdministratorContent(self,RoleName,Description):
    ...
    def createCreateAdministratorContentMain(self):  
        try:   
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            sheet=book.sheet_by_name('Role')
            cell = sheet.cell(296,5)
            RoleName = cell.value
            cell = sheet.cell(297,5)
            Description = cell.value
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            first_sheet = book.sheet_by_name('UserAssignToRole')
            cell = first_sheet.cell(4,1)
            FirstName = cell.value
            obj2= RoleCreateAdministratorContent()
            obj2.createCreateAdministratorContent(RoleName,Description) 
...
if __name__ == '__main__':
    ...
    obj11= CreateAdministratorContent()
    obj12= BaseTestClass()
    obj12.UserLogin()
    obj11.createCreateAdministratorContentMain()
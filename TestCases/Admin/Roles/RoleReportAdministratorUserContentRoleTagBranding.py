...
class RoleReportAdministratorUserContentRoleTagBranding():
    
    def createReportAdministratorUserContentRoleTagBranding(self,RoleName,Description):
    ...
    def createReportAdministratorUserContentRoleTagBrandingMain(self):  
        from BaseTestClass import excelPath
        try:   
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            sheet=book.sheet_by_name('Role')
            cell = sheet.cell(364,1)
            RoleName = cell.value
            cell = sheet.cell(365,1)
            Description = cell.value
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            first_sheet = book.sheet_by_name('UserAssignToRole')
            cell = first_sheet.cell(2,1)
            FirstName = cell.value
            obj2= RoleReportAdministratorUserContentRoleTagBranding()
            obj2.createReportAdministratorUserContentRoleTagBranding(RoleName,Description)  
...
if __name__ == '__main__':
    ...
    obj11= ReportAdministratorUserContentRoleTagBranding()
    obj12= BaseTestClass()
    obj12.UserLogin()
    obj11.createReportAdministratorUserContentRoleTagBrandingMain()
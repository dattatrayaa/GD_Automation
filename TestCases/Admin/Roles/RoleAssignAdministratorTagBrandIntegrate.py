...
class RoleAssignAdministratorTagBrandIntegrate():
    
    def createAssignAdministratorTagBrandIntegrate(self,RoleName,Description):
    ...
    def createAssignAdministratorTagBrandIntegrateMain(self):
        from BaseTestClass import excelPath  
        try:   
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            sheet=book.sheet_by_name('Role')
            cell = sheet.cell(320,1)
            RoleName = cell.value
            cell = sheet.cell(321,1)
            Description = cell.value
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            first_sheet = book.sheet_by_name('UserAssignToRole')
            cell = first_sheet.cell(1,1)
            FirstName = cell.value
            obj2= RoleAssignAdministratorTagBrandIntegrate()
            obj2.createAssignAdministratorTagBrandIntegrate(RoleName,Description)  
...
if __name__ == '__main__':
    ...
    obj11= AssignAdministratorTagBrandIntegrate()
    obj12= BaseTestClass()
    obj12.UserLogin()
    obj11.createAssignAdministratorTagBrandIntegrateMain()
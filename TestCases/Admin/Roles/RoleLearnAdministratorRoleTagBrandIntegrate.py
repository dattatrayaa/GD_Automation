...
class RoleLearnAdministratorRoleTagBrandIntegrate():
    
    def createLearnAdministratorRoleTagBrandIntegrateRole(self,RoleName,Description):
    ...
    def createLearnAdministratorRoleTagBrandIntegrateMain(self):  
        try:   
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            sheet=book.sheet_by_name('Role')
            cell = sheet.cell(187,5)
            RoleName = cell.value
            cell = sheet.cell(188,5)
            Description = cell.value
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            first_sheet = book.sheet_by_name('UserAssignToRole')
            cell = first_sheet.cell(3,1)
            FirstName = cell.value
            obj2= RoleLearnAdministratorRoleTagBrandIntegrate()
            obj2.createLearnAdministratorRoleTagBrandIntegrateRole(RoleName,Description)  
...
if __name__ == '__main__':
    ...
    obj11= LearnAdministratorRoleTagBrandIntegrate()
    obj12= BaseTestClass()
    obj12.UserLogin()
    obj11.createLearnAdministratorRoleTagBrandIntegrateMain()
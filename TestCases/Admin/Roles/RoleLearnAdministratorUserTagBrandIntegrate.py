...
class RoleLearnAdministratorUserTagBrandIntegrate():
    
    def createLearnAdministratorUserTagBrandIntegrate(self,RoleName,Description):
    ...
    def createLearnAdministratorUserTagBrandIntegrateMain(self):  
        try:   
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            sheet=book.sheet_by_name('Role')
            cell = sheet.cell(235,1)
            RoleName = cell.value
            cell = sheet.cell(236,1)
            Description = cell.value
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            first_sheet = book.sheet_by_name('UserAssignToRole')
            cell = first_sheet.cell(3,1)
            FirstName = cell.value
            obj2= RoleLearnAdministratorUserTagBrandIntegrate()
            obj2.createLearnAdministratorUserTagBrandIntegrate(RoleName,Description)  
...
if __name__ == '__main__':
    ...
    obj11= LearnAdministratorUserTagBrandIntegrate()
    obj12= BaseTestClass()
    obj12.UserLogin()
    obj11.createLearnAdministratorUserTagBrandIntegrateMain()
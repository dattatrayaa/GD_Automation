...
class RoleReportAdministratorUserContentRoleTag():
    
    def createReportAdministratorUserContentRoleTag(self,RoleName,Description):
    ...
    def createReportAdministratorUserContentRoleTagMain(self):  
        try:   
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            sheet=book.sheet_by_name('Role')
            cell = sheet.cell(364,5)
            RoleName = cell.value
            cell = sheet.cell(365,5)
            Description = cell.value
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            first_sheet = book.sheet_by_name('UserAssignToRole')
            cell = first_sheet.cell(2,1)
            FirstName = cell.value
            obj2= RoleReportAdministratorUserContentRoleTag()
            obj2.createReportAdministratorUserContentRoleTag(RoleName,Description)  
...
if __name__ == '__main__':
    ...
    obj11= ReportAdministratorUserContentRoleTag()
    obj12= BaseTestClass()
    obj12.UserLogin()
    obj11.createReportAdministratorUserContentRoleTagMain()
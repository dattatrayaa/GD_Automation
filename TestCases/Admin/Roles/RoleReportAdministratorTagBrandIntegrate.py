...
class RoleReportAdministratorTagBrandIntegrate():
    
    def createReportAdministratorTagBrandIntegrate(self,RoleName,Description):
    ...
    def createReportAdministratorTagBrandIntegrateMain(self):  
        from BaseTestClass import excelPath
        try:   
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            sheet=book.sheet_by_name('Role')
            cell = sheet.cell(376,5)
            RoleName = cell.value
            cell = sheet.cell(377,5)
            Description = cell.value
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            first_sheet = book.sheet_by_name('UserAssignToRole')
            cell = first_sheet.cell(2,1)
            FirstName = cell.value
            obj2= RoleReportAdministratorTagBrandIntegrate()
            obj2.createReportAdministratorTagBrandIntegrate(RoleName,Description)  
...
if __name__ == '__main__':
    ...
    obj11= ReportAdministratorTagBrandIntegrate()
    obj12= BaseTestClass()
    obj12.UserLogin()
    obj11.createReportAdministratorTagBrandIntegrateMain()
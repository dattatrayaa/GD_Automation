...
class RoleLearnAdministratorUserContentRoleTag():
    
    def createLearnAdministratorUserContentRoleTagRole(self,RoleName,Description):
    ...
    def createLearnAdministratorUserContentRoleTagMain(self):  
         try:   
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            sheet=book.sheet_by_name('Role')
            cell = sheet.cell(166,5)
            RoleName = cell.value
            cell = sheet.cell(167,6)
            Description = cell.value
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            first_sheet = book.sheet_by_name('UserAssignToRole')
            cell = first_sheet.cell(3,1)
            FirstName = cell.value
            obj2= RoleLearnAdministratorUserContentRoleTag()
            obj2.createLearnAdministratorUserContentRoleTagRole(RoleName,Description)  
...
if __name__ == '__main__':
    ...
    obj11= LearnAdministratorUserContentRoleTag()
    obj12= BaseTestClass()
    obj12.UserLogin()
    obj11.createLearnAdministratorUserContentRoleTagMain()
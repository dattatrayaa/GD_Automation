...
class RoleReportAdministratorRoleTagBrandIntegrate():
    
    def createReportAdministratorRoleTagBrandIntegrate(self,RoleName,Description):
    ...
    def createReportAdministratorRoleTagBrandIntegrateMain(self):  
        try:   
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            sheet=book.sheet_by_name('Role')
            cell = sheet.cell(376,1)
            RoleName = cell.value
            cell = sheet.cell(377,1)
            Description = cell.value
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            first_sheet = book.sheet_by_name('UserAssignToRole')
            cell = first_sheet.cell(2,1)
            FirstName = cell.value
            obj2= RoleReportAdministratorRoleTagBrandIntegrate()
            obj2.createReportAdministratorRoleTagBrandIntegrate(RoleName,Description)  
...
if __name__ == '__main__':
    ...
    obj11= ReportAdministratorRoleTagBrandIntegrate()
    obj12= BaseTestClass()
    obj12.UserLogin()
    obj11.createReportAdministratorRoleTagBrandIntegrateMain()
...
class RoleCreateAdministratorContentTagBrandIntegrate():
    
    def createCreateAdministratorContentTagBrandIntegrate(self,RoleName,Description):
    ...
    def createCreateAdministratorContentTagBrandIntegrateMain(self):
        from BaseTestClass import excelPath  
        try:   
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            sheet=book.sheet_by_name('Role')
            cell = sheet.cell(303,1)
            RoleName = cell.value
            cell = sheet.cell(304,1)
            Description = cell.value
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            first_sheet = book.sheet_by_name('UserAssignToRole')
            cell = first_sheet.cell(4,1)
            FirstName = cell.value
            obj2= RoleCreateAdministratorContentTagBrandIntegrate()
            obj2.createCreateAdministratorContentTagBrandIntegrate(RoleName,Description) 
...
if __name__ == '__main__':
    ...
    obj11= CreateAdministratorContentTagBrandIntegrate()
    obj12= BaseTestClass()
    obj12.UserLogin()
    obj11.createCreateAdministratorContentTagBrandIntegrateMain()
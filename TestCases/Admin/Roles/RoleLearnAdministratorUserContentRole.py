...
class RoleLearnAdministratorUserContentRole():
    
    def createLearnAdministratorUserContentRoleRole(self,RoleName,Description):
    ...
    def createLearnAdministratorUserContentRoleMain(self):  
        try:   
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            sheet=book.sheet_by_name('Role')
            cell = sheet.cell(170,5)
            RoleName = cell.value
            cell = sheet.cell(171,5)
            Description = cell.value
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            first_sheet = book.sheet_by_name('UserAssignToRole')
            cell = first_sheet.cell(3,1)
            FirstName = cell.value
            obj2= RoleLearnAdministratorUserContentRole()
            obj2.createLearnAdministratorUserContentRoleRole(RoleName,Description)  
...
if __name__ == '__main__':
    ...
    obj11= LearnAdministratorUserContentRole()
    obj12= BaseTestClass()
    obj12.UserLogin()
    obj11.createLearnAdministratorUserContentRoleMain()
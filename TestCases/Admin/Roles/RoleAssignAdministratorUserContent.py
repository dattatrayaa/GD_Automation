...
class RoleAssignAdministratorUserContent():
    
    def createAssignAdministratorUserContent(self,RoleName,Description):
    ...
    def createAssignAdministratorUserContentMain(self):  
        try:   
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            sheet=book.sheet_by_name('Role')
            cell = sheet.cell(312,1)
            RoleName = cell.value
            cell = sheet.cell(313,1)
            Description = cell.value
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            first_sheet = book.sheet_by_name('UserAssignToRole')
            cell = first_sheet.cell(1,1)
            FirstName = cell.value
            obj2= RoleAssignAdministratorUserContent()
            obj2.createAssignAdministratorUserContent(RoleName,Description)
...
if __name__ == '__main__':
    ...
    obj11= AssignAdministratorUserContent()
    obj12= BaseTestClass()
    obj12.UserLogin()
    obj11.createAssignAdministratorUserContentMain()
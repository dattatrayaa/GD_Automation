...
class RoleCreateAdministratorAll():
    
    def createCreateAdministratorAll(self,RoleName,Description):
    ...
    def createCreateAdministratorAllMain(self):  
        try:   
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            sheet=book.sheet_by_name('Role')
            cell = sheet.cell(201,1)
            RoleName = cell.value
            cell = sheet.cell(202,1)
            Description = cell.value
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            first_sheet = book.sheet_by_name('UserAssignToRole')
            cell = first_sheet.cell(4,1)
            FirstName = cell.value
            obj2= RoleCreateAdministratorAll()
            obj2.createCreateAdministratorAll(RoleName,Description)
...
if __name__ == '__main__':
    ...
    obj11= CreateAdministratorAll()
    obj12= BaseTestClass()
    obj12.UserLogin()
    obj11.createCreateAdministratorAllMain()
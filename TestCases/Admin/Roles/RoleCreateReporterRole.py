...
class RoleCreateReporterRole():
    
    def createCreateReportRole(self,RoleName,Description):
    ...
    def createCreateReportRoleMain(self):  
        try:   
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            sheet=book.sheet_by_name('Role')
            cell = sheet.cell(196,1)
            RoleName = cell.value
            cell = sheet.cell(197,1)
            Description = cell.value
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            first_sheet = book.sheet_by_name('UserAssignToRole')
            cell = first_sheet.cell(4,1)
            FirstName = cell.value
            obj2= RoleCreateReporterRole()
            obj2.createCreateReportRole(RoleName,Description)  
...
if __name__ == '__main__':
    ...
    obj11= CreateReportRole()
    obj12= BaseTestClass()
    obj12.UserLogin()
    obj11.createCreateReportRoleMain()
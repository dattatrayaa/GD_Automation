...
class RoleCreateAdministratorBrandIntegrate():
    
    def createCreateAdministratorBrandIntegrate(self,RoleName,Description):
    ...
    def createCreateAdministratorBrandIntegrateMain(self):  
        try:   
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            sheet=book.sheet_by_name('Role')
            cell = sheet.cell(264,5)
            RoleName = cell.value
            cell = sheet.cell(265,5)
            Description = cell.value
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            first_sheet = book.sheet_by_name('UserAssignToRole')
            cell = first_sheet.cell(4,1)
            FirstName = cell.value
            obj2= RoleCreateAdministratorBrandIntegrate()
            obj2.createCreateAdministratorBrandIntegrate(RoleName,Description) 
...
if __name__ == '__main__':
    ...
    obj11= CreateAdministratorBrandIntegrate()
    obj12= BaseTestClass()
    obj12.UserLogin()
    obj11.createCreateAdministratorBrandIntegrateMain()
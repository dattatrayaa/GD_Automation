...
class RoleReportAdministratorUserContentTagBrand():
    
    def createReportAdministratorUserContentTagBrand(self,RoleName,Description):
    ...
    def createReportAdministratorUserContentTagBrandMain(self): 
        from BaseTestClass import excelPath
        try:   
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            sheet=book.sheet_by_name('Role')
            cell = sheet.cell(400,1)
            RoleName = cell.value
            cell = sheet.cell(401,1)
            Description = cell.value
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            first_sheet = book.sheet_by_name('UserAssignToRole')
            cell = first_sheet.cell(2,1)
            FirstName = cell.value
            obj2= RoleReportAdministratorUserContentTagBrand()
            obj2.createReportAdministratorUserContentTagBrand(RoleName,Description)  
...
if __name__ == '__main__':
    ...
    obj11= ReportAdministratorUserContentTagBrand()
    obj12= BaseTestClass()
    obj12.UserLogin()
    obj11.createReportAdministratorUserContentTagBrandMain()
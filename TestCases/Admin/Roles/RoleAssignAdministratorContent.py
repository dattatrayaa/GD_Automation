...
class RoleAssignAdministratorContent():
    
    def createAssignAdministratorContent(self,RoleName,Description):
    ...
    def createAssignAdministratorContentMain(self):  
        try:   
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            sheet=book.sheet_by_name('Role')
            cell = sheet.cell(356,1)
            RoleName = cell.value
            cell = sheet.cell(357,1)
            Description = cell.value
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            first_sheet = book.sheet_by_name('UserAssignToRole')
            cell = first_sheet.cell(1,1)
            FirstName = cell.value
            obj2= RoleAssignAdministratorContent()
            obj2.createAssignAdministratorContent(RoleName,Description) 
...
if __name__ == '__main__':
    ...
    obj11= AssignAdministratorContent()
    obj12= BaseTestClass()
    obj12.UserLogin()
    obj11.createAssignAdministratorContentMain()
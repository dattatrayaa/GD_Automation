...
class RoleAssignAdministratorRole():
    
    def createAssignAdministratorRole(self,RoleName,Description):
    ...
    def createAssignAdministratorRoleMain(self):
        from BaseTestClass import excelPath  
        try:   
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            sheet=book.sheet_by_name('Role')
            cell = sheet.cell(332,1)
            RoleName = cell.value
            cell = sheet.cell(333,1)
            Description = cell.value
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            first_sheet = book.sheet_by_name('UserAssignToRole')
            cell = first_sheet.cell(1,1)
            FirstName = cell.value
            obj2= RoleAssignAdministratorRole()
            obj2.createAssignAdministratorRole(RoleName,Description)
...
if __name__ == '__main__':
    ...
    obj11= AssignAdministratorRole()
    obj12= BaseTestClass()
    obj12.UserLogin()
    obj11.createAssignAdministratorRoleMain()
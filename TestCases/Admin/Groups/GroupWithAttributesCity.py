...
class GroupWithAttributesCity:
    
    def create(self,FirstName,LastName,Email,EmployeeId,Password,cityName,attributeName):
    ...
    def groupCreateForCampaign(self,groupName,FirstName,attributeName,cityName):
        driver.refresh()
        wait=WebDriverWait(driver, 60)
    ...
    def CreateGroupWithAttributeCity(self):
        ...
        cell1 = first_sheet.cell(7,1)
        cityName = cell1.value
        cityNameId = cityName.split("_")
        emp = cityNameId[0]+"_"
        ids = cityNameId[1]
        empId = int(ids)+1
        cityNameUpdated= emp+str(empId)
        
        cell1 = first_sheet.cell(8,1)
        ...
        sheet.cell(row=3, column=2).value = FirstNameUpdated
        sheet.cell(row=4, column=2).value = LastNameUpdated
        sheet.cell(row=5, column=2).value = EmailIdUpdated
        sheet.cell(row=6, column=2).value = EmployeeIdUpdated
        sheet.cell(row=8, column=2).value = cityNameUpdated
        sheet.cell(row=9, column=2).value = groupNameUpdated
        ...
        try:
            t=GroupWithAttributesCity()
            print "\nCreating New User\n"
            t.create(FirstName, LastName, Email, EmployeeId, Password, cityName, attributeName)
            
            print "\nCreating Group with '"+cityName+"' Attribute\n"
            t.groupCreateForCampaign(groupName, FirstName, attributeName, cityName)
...
if __name__=='__main__':
    ...
    gr=GroupWithAttributesCity()
    gr.CreateGroupWithAttributeCity()

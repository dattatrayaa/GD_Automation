...
class GroupWithAttributesRegion:
    def create(self,FirstName,LastName,Email,EmployeeId,Password,region,attributeName):
    ...
    def groupCreateForCampaign(self,groupName,FirstName,attributeName,region):
        driver.refresh()
        wait=WebDriverWait(driver, 60)
        ...
        group.OverViewContainer(region)
    ...
    def CreateGroupWithAttributeRegion(self):
        book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
        first_sheet = book.sheet_by_name('Groups')
        
        cell = first_sheet.cell(66,1)
        ...
        sheet.cell(row=67, column=2).value = FirstNameUpdated
        sheet.cell(row=68, column=2).value = LastNameUpdated
        sheet.cell(row=69, column=2).value = EmailIdUpdated
        sheet.cell(row=70, column=2).value = EmployeeIdUpdated
        sheet.cell(row=72, column=2).value = regionIdUpdated
        sheet.cell(row=73, column=2).value = groupNameUpdated
        ...
        try:
            t=GroupWithAttributesRegion()
            print "\nCreating New User\n"
            t.create(FirstName, LastName, Email, EmployeeId, Password, region, attributeName)
            
            print "\nCreating Group with '"+region+"' Attribute\n"
            t.groupCreateForCampaign(groupName, FirstName, attributeName, region)
...
if __name__=='__main__':
    ...
    gr=GroupWithAttributesRegion()
    gr.CreateGroupWithAttributeRegion()

...
class GroupWithAttributesCountry:
    
    def create(self,FirstName,LastName,Email,EmployeeId,Password,countryName,attributeName):
    ...
    def groupCreateForCampaign(self,groupName,FirstName,attributeName,countryName):
        driver.refresh()
        wait=WebDriverWait(driver, 60)
        ...
        group.OverViewContainer(countryName)
    ...
    def CreateGroupWithAttributeCountry(self):
        ...
        sheet.cell(row=13, column=2).value = FirstNameUpdated
        sheet.cell(row=14, column=2).value = LastNameUpdated
        sheet.cell(row=15, column=2).value = EmailIdUpdated
        sheet.cell(row=16, column=2).value = EmployeeIdUpdated
        sheet.cell(row=18, column=2).value = countryNameUpdated
        sheet.cell(row=19, column=2).value = groupNameUpdated
        ...
        try:
            t=GroupWithAttributesCountry()
            print "\nCreating New User\n"
            t.create(FirstName, LastName, Email, EmployeeId, Password, countryName, attributeName)
            
            print "\nCreating Group with '"+countryName+"' Attribute\n"
            t.groupCreateForCampaign(groupName, FirstName, attributeName, countryName)
...
if __name__=='__main__':
    ...
    gr=GroupWithAttributesCountry()
    gr.CreateGroupWithAttributeCountry()

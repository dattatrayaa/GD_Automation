...
class RoleLearnAssignCreateReportAdministratorIntegrate():
    
    def createLearnAssignCreateReportAdministratorIntegrate(self,RoleName,Description):
    ...
    def createLearnAssignCreateReportAdministratorIntegrateMain(self):
        from BaseTestClass import excelPath  
        try:   
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            sheet=book.sheet_by_name('Role')
            cell = sheet.cell(436,1)
            RoleName = cell.value
            cell = sheet.cell(437,1)
            Description = cell.value
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            first_sheet = book.sheet_by_name('UserAssignToRole')
            cell = first_sheet.cell(3,1)
            FirstName = cell.value
            obj2= RoleLearnAssignCreateReportAdministratorIntegrate()
            obj2.createLearnAssignCreateReportAdministratorIntegrate(RoleName,Description)  
...
if __name__ == '__main__':
    ...
    obj11= LearnAssignCreateReportAdministratorIntegrate()
    obj12= BaseTestClass()
    obj12.UserLogin()
    obj11.createLearnAssignCreateReportAdministratorIntegrateMain()
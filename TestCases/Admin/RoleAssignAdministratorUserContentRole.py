...
class RoleAssignAdministratorUserContentRole():
    
    def createAssignAdministratorUserContentRole(self,RoleName,Description):
    ...
    def createAssignAdministratorUserContentRoleMain(self):
        from BaseTestClass import excelPath  
        try:   
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            sheet=book.sheet_by_name('Role')
            cell = sheet.cell(308,5)
            RoleName = cell.value
            cell = sheet.cell(309,5)
            Description = cell.value
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            first_sheet = book.sheet_by_name('UserAssignToRole')
            cell = first_sheet.cell(1,1)
            FirstName = cell.value
            obj2= RoleAssignAdministratorUserContentRole()
            obj2.createAssignAdministratorUserContentRole(RoleName,Description)
...
if __name__ == '__main__':
    ...
    obj11= AssignAdministratorUserContentRole()
    obj12= BaseTestClass()
    obj12.UserLogin()
    obj11.createAssignAdministratorUserContentRoleMain()
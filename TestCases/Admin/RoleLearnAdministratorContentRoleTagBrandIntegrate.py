...
class RoleLearnAdministratorContentRoleTagBrandIntegrate():
    
    def createLearnAdministratorContentRoleTagBrandIntegrateRole(self,RoleName,Description):
    ...
    def createLearnAdministratorContentRoleTagBrandIntegrateMain(self):
        from BaseTestClass import excelPath  
        try:   
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            sheet=book.sheet_by_name('Role')
            cell = sheet.cell(183,5)
            RoleName = cell.value
            cell = sheet.cell(184,5)
            Description = cell.value
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            first_sheet = book.sheet_by_name('UserAssignToRole')
            cell = first_sheet.cell(3,1)
            FirstName = cell.value
            obj2= RoleLearnAdministratorContentRoleTagBrandIntegrate()
            obj2.createLearnAdministratorContentRoleTagBrandIntegrateRole(RoleName,Description) 
...
if __name__ == '__main__':
    ...
    obj11= LearnAdministratorContentRoleTagBrandIntegrate()
    obj12= BaseTestClass()
    obj12.UserLogin()
    obj11.createLearnAdministratorContentRoleTagBrandIntegrateMain()
...
class RoleLearnAssignCreateReportAdministratorAllRole():
    
    def createLearnAssignCreateReportAdministratorAllRole(self,RoleName,Description):
        ...
        #elem=driver.find_element_by_xpath("html/body/div/div/div[3]/div[2]/div/header/div/button[2]")
    ...
    def createLearnAssignCreateReportAdministratorAllMain(self):
        from BaseTestClass import excelPath  
        try:   
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            sheet=book.sheet_by_name('Role')
            cell = sheet.cell(219,1)
            RoleName = cell.value
            cell = sheet.cell(220,1)
            Description = cell.value
            book=xlrd.open_workbook(os.path.join('Test_Data/TestData.xlsx'))
            first_sheet = book.sheet_by_name('UserAssignToRole')
            cell = first_sheet.cell(3,1)
            FirstName = cell.value
            obj2= RoleLearnAssignCreateReportAdministratorAllRole()
            obj2.createLearnAssignCreateReportAdministratorAllRole(RoleName,Description)  
...
if __name__ == '__main__':
    ...
    obj11= LearnAssignCreateReportAdministratorAllRole()
    obj12= BaseTestClass()
    obj12.UserLogin()
    obj11.createLearnAssignCreateReportAdministratorAllMain()
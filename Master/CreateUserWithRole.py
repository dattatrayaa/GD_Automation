...
class CreateUserWithRole:
    ...
    def create(self,FirstName,LastName,Email,EmployeeId,Password,role):
        ...
        driver.find_element_by_xpath("//*[@id='search-users']").send_keys(FirstName)
        print "Searching for the Created User"
        
        wait.until(EC.visibility_of_element_located((By.XPATH,"//tbody/tr[1]/td[2]")))
        ele =driver.find_element_by_xpath("//tbody/tr[1]/td[2]").text
        if(ele==FirstName):
    ...
    def againLoginUser(self,url,username,password):
        driver.get(url)
        element = WebDriverWait(driver, 60).until(EC.presence_of_element_located((By.ID, "password")))
